function findCaliforniaCafes(searchTerm) {
    ...
    const results = filteredCafes.map(filteredCafe => {
        const placeDetails = places.find(place => place.id === filteredCafe.place_id);
        return {
            cafeName: filteredCafe.name,
            placeDetails
        };
    });
...
const cafeResults = findCaliforniaCafes(searchTerm);
